const HOLD_ON_ENTER = new Set(['Confirmed'])
const HOLD_RELEASE  = new Set(['Cancelled'])
const STOCK_DEDUCT  = new Set(['Delivered'])
...
export default defineEventHandler(async (event) => {
  ...
  if (Object.keys(patch).length) {
    const { error } = await supabase.from('orders').update(patch).eq('id', id).eq('org_id', orgId)
