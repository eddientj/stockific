export default defineEventHandler(async (event) => {
  ...
  // Same length window before that (for comparison)
  const prevSince = new Date(since.getTime() - months * 30.44 * 86400000).toISOString()

  // Orders in current period (non-cancelled) with items + product → category
  const { data: orders } = await supabase
    .from('orders')
    .select(`
      id, status, shipping, created_at,
      order_items(name, qty, price, product_id, product:products(name, categories(name)))
    `)
  ...
  const prevTotal = (prevOrders ?? []).reduce((s: number, o: any) => {
    return s + (o.order_items ?? []).reduce((ss: number, i: any) => ss + i.qty * i.price, 0) + (o.shipping ?? 0)
  }, 0)
  ...
  for (const o of withTotal) {
    for (const item of (o.order_items ?? []) as any[]) {
      ...
      const category = item.product?.categories?.name ?? 'Uncategorised'
      const prev     = productMap.get(key) ?? { name, category, revenue: 0, units: 0 }
